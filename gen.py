...
function buildLOI(){
  ...
  allPartners.forEach(p=>{const n=nodes[p];if(!n)return;
    ...
    const accentCols=connInit
      ? [...new Set(CONNECTIONS.filter(c=>c.type==='Partner'&&c.from===p&&nodes[c.to]).map(c=>CC[ELEMENTS[c.to]?.category||'']||'rgba(255,255,255,.2)'))]
      : ['rgba(255,255,255,.2)'];
    ...
    grp.appendChild(bg);
    drawAccentBar(grp,x0,n.y-chipH/2,chipH,accentCols);
    grp.appendChild(t);
...
// ── Shared partner helpers (used by both buildLOI and buildNet) ──────────
function partnerColors(label){
  const cats=[...new Set(
    CONNECTIONS
      .filter(c=>c.type==='Partner'&&c.from===label&&ELEMENTS[c.to])
      .map(c=>ELEMENTS[c.to]?.category||'')
      .filter(Boolean)
  )];
  return cats.length ? cats.map(c=>CC[c]||'rgba(255,255,255,.2)') : ['rgba(255,255,255,.2)'];
}
function drawAccentBar(svgGroup,x0,y0,chipH,colors){
  ...
  colors.forEach((col,i)=>{
    ...
    const seg=svgEl('rect',{
      x:x0, y:sy,
      width:3, height:segH+(i<n-1?0.5:0),
      rx: i===0||i===n-1 ? '2' : '0',
...
function buildNet(){
  ...
  function lp(deg){
    ...
    const d=((deg-270)+360)%360; // distance from 270
    const mirror=d>180?360-d:d;  // symmetric distance
    if(mirror<gap){
      const push=(deg<270||(deg>340||deg<90))?-gap*1.2:gap*1.2;
      deg=((deg+push)+360)%360;
    }
  ...
  function placeCircle(keys,rad,gapDeg,gapWidth){
    // gapDeg: centre of exclusion zone (degrees), gapWidth: total arc to skip
    const sorted=[...keys].sort((a,b)=>(ELEMENTS[a]?.size||0)-(ELEMENTS[b]?.size||0));
    const n=sorted.length,angs={};
    if(!gapDeg&&gapDeg!==0){
      // No gap: evenly spaced
      const step=360/n;
      sorted.forEach((k,i)=>{const deg=30+i*step,a=deg*Math.PI/180;pos[k]={x:cx+rad*Math.sin(a),y:cy-rad*Math.cos(a)};angs[k]=deg;});
    } else {
      // Distribute n items around 360° with a gap arc excluded around gapDeg
      const gap=gapWidth||40; // degrees to leave empty
      const available=360-gap;
      const step=available/(n);
      // Start placing just after the gap
      const startDeg=((gapDeg+gap/2)+40)%360;
      sorted.forEach((k,i)=>{
        const deg=(startDeg+i*step)%360;
        const a=deg*Math.PI/180;
        pos[k]={x:cx+rad*Math.sin(a),y:cy-rad*Math.cos(a)};
        angs[k]=deg;
      });
    }
    return angs;
  ...
  Object.entries(initPreferred).forEach(([k,deg])=>{
    if(initAngs[k]!=null){initAngs[k]=deg;}
  });

  // Step 2: detect and resolve conflicts — iteratively push overlapping
  // initiatives apart, preserving preferred positions where possible
  function resolveOverlaps(angs, preferred, minSep, maxIter=80){
    const keys=Object.keys(angs);
    for(let iter=0;iter<maxIter;iter++){
      let moved=false;
      // Sort by angle for neighbour comparison
      const sorted=keys.slice().sort((a,b)=>angs[a]-angs[b]);
      for(let i=0;i<sorted.length;i++){
        const a=sorted[i], b=sorted[(i+1)%sorted.length];
        const gap=((angs[b]-angs[a])+360)%360;
        const fullGap=((angs[sorted[0]]-angs[sorted[sorted.length-1]])+360)%360;
        if(gap<minSep&&gap!==0){
          const push=(minSep-gap)/2;
          // Don't move preferred items unless necessary — move the non-preferred one more
          const aPreferred=preferred[a]!=null, bPreferred=preferred[b]!=null;
          const aShare=aPreferred&&!bPreferred?0.1:bPreferred&&!aPreferred?0.9:0.5;
          angs[a]=((angs[a]-push*aShare)+360)%360;
          angs[b]=(angs[b]+push*(1-aShare)+360)%360;
          moved=true;
        }
      }
      if(!moved)break;
  ...
  Object.keys(initAngs).forEach(k=>{
    ...
    if(dist<gapHalfInit){
      ...
      const toRight=(gapCentre+gapHalfInit+2+360)%360;
      const toLeft=(gapCentre-gapHalfInit-2+360)%360;
      const dRight=Math.min((toRight-deg+360)%360,(deg-toRight+360)%360);
      const dLeft=Math.min((toLeft-deg+360)%360,(deg-toLeft+360)%360);
      initAngs[k]=dRight<dLeft?toRight:toLeft;
    }
  });
  Object.keys(initAngs).forEach(k=>{
    const a=initAngs[k]*Math.PI/180;
    pos[k]={x:cx+initR*Math.sin(a),y:cy-initR*Math.cos(a)};
  });
  ...
  // ── Adaptive output placement ─────────────────────────────────────────
  // Cluster outputs around their parent initiative, then resolve overlaps.
  // Uses same resolveOverlaps() as initiatives — no preferred positions,
  // all outputs treated equally, min separation ~7° (~45px at r=368).
  const outByParent={};
  outKeys.forEach(ok=>{
    const ang=idealAng(ok).toFixed(1);
  ...
  outKeys.forEach(ok=>{
    const parentAng=idealAng(ok);
    const siblings=outByParent[parentAng.toFixed(1)];
    const idx=siblings.indexOf(ok);
    const n=siblings.length;
    const offset=(idx-(n-1)/2)*spreadStep;
    outAngs[ok]=(parentAng+offset+360)%360;
  });
  // Resolve overlaps across ALL outputs (not just within groups)
  resolveOverlaps(outAngs,{},minOutSep);
  // Apply gap zone push
  Object.keys(outAngs).forEach(k=>{
    let deg=outAngs[k];
    const dist=Math.min((deg-outGapCentre+360)%360,(outGapCentre-deg+360)%360);
    if(dist<outGapHalf){
      deg=deg<outGapCentre?(outGapCentre-outGapHalf-2+360)%360:(outGapCentre+outGapHalf+2)%360;
      outAngs[k]=deg;
    }
    const a=deg*Math.PI/180;
  ...
  CONNECTIONS.forEach(c=>{
    ...
    const cat=fe.category||te.category||'';
    // Partner lines: faint category tint at rest (matching slide 3 style)
    const col=c.type==='Partner'
      ? (CC[cat]?CC[cat]+'44':'rgba(255,255,255,0.12)')
      : (CC[cat]||'#8898aa')+'88';
    const dash=c.type==='Partner'?'4,4':c.type==='Output'?'5,4':'none';
    const sz=c.size||1;
    // Partner lines match slide 3 weight (sw=1); other connections use size-based width
    const sw=c.type==='Partner'?'1':(0.3+sz*3).toFixed(1);
    const af=pos[c.from],at=pos[c.to];if(!af||!at)return;

    // Compute edge point: move from element centre towards the other element
    // by the element's bounding radius, so lines connect at the nearest surface.
    function edgePt(p, el, towards){
      const dx=towards.x-p.x, dy=towards.y-p.y;
      const dist=Math.sqrt(dx*dx+dy*dy)||1;
      const ux=dx/dist, uy=dy/dist;
      let r=0;
      if(!el) return p;
      if(el.type==='Cohort') r=36+(el.size-8)*8;
      else if(el.type==='Lever') r=18;
      else if(el.type==='Initiative') r=14; // half of 28px square (approx)
      else if(el.type==='Output') r=10;
      else if(el.type==='Context') r=70;
      return {x:p.x+ux*r, y:p.y+uy*r};
    }
    const ef=edgePt(af,fe,at);
    const et=edgePt(at,te,af);
  ...
  function netClick(label){
    ...
    edgeG.querySelectorAll('path').forEach(l=>{
      const f=l.getAttribute('data-from'),t=l.getAttribute('data-to');
      const ok=conn.has(f)&&conn.has(t);
      const isPartner=(ELEMENTS[f]?.type==='Partner'||ELEMENTS[t]?.type==='Partner');
      if(ok){
        l.setAttribute('opacity','0.95');
        // Partner lines: boost to 2.5 + restore category colour (matching slide 3 behaviour)
        if(isPartner){
          ...
          l.setAttribute('stroke-width',(parseFloat(l._sw||2)*1.3).toFixed(1));
        }
      } else {
        l.setAttribute('opacity','0.03');
  ...
  netResetFn=function(){
    edgeG.querySelectorAll('path').forEach(l=>{
      l.setAttribute('stroke-width',l._sw||'2');
      // Restore partner line stroke colour to category-tinted faint version
      const f=l.getAttribute('data-from'),t=l.getAttribute('data-to');
      if(ELEMENTS[f]?.type==='Partner'||ELEMENTS[t]?.type==='Partner'){
        const init=ELEMENTS[f]?.type==='Partner'?t:f;
        const cat=ELEMENTS[init]?.category||'';
        const col=CC[cat]?CC[cat]+'44':'rgba(255,255,255,0.12)';
  ...
  function partnerColor(label){return partnerColors(label)[0];}
  ...
  function drawNode(key){
    ...
    if(el.type==='Context'){
      ...
      const accentCols=partnerColors(key);
      ...
      grp.appendChild(bg);
      drawAccentBar(grp,x0,p.y-chipH/2,chipH,accentCols);
